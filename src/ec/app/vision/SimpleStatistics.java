...
public class SimpleStatistics extends Statistics implements SteadyStateStatisticsForm //, ec.eval.ProvidesBestSoFar
    ...
    public void postEvaluationStatistics(final EvolutionState state)
        ...
        for(int x=0;x<state.population.subpops.length;x++)
            ...
            if (best_of_run[x]==null || ((SharedFitness) best_i[x].fitness).compareToOther((SharedFitness) best_of_run[x].fitness))
                best_of_run[x] = (Individual)(best_i[x].clone());
            }
        
        Individual[] pop = state.population.subpops[0].individuals;
        ...
        state.output.print("" + max, statisticslog);
        ...
        state.output.print("" + (total / pop.length), statisticslog);
        state.output.print(",", statisticslog);
        ...
        state.output.print("" + max_score, statisticslog);
        state.output.print(",", statisticslog);
        
        int total_scores = 0;
        for (Individual ind : pop) {
        	SharedFitness f = (SharedFitness) ind.fitness;
        	total_scores += f.score;
        }
        
        state.output.println("" + (total_scores / pop.length), statisticslog);
